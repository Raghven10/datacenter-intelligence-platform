...
def list_equipments(request: Request, user=Depends(require_user), db: Session = Depends(get_db)):
    log_action(db, "FETCH", target_table="equipments", description="Viewed equipment inventory")
...
def create_equipment(
    ...
    # Auto-create EquipmentType if not exists
    ...
    # Create Equipment with provided Place ID
    eq = Equipment(
        name=name,
    ...
    db.add(eq)
    log_action(db, "CREATE", target_table="equipments", description=f"Provisioned new asset: {name}")
    db.commit()
    db.refresh(eq)

    # Add metadata
    if any([pressure, temperature, voltage, frequency]):
    ...
    # Trigger alert if unserviceable
    if serviceability == "US":
        NotificationManager.broadcast_to_role(db, "sysadmin", "Critical Asset Alert", 
                                              f"A new asset '{name}' has been provisioned as UNSERVICEABLE.", "error")
        db.commit()
...
def edit_equipment(
    ...
    db.add(meta)
    log_action(db, "UPDATE", target_table="equipments", target_id=eq_id, description=f"Updated equipment configuration: {name}")
    db.commit()
    ...
    if serviceability == "US":
        # Notify admins
        NotificationManager.broadcast_to_role(db, "sysadmin", "Critical Asset Alert", 
                                              f"Asset '{name}' has been marked as UNSERVICEABLE.", "error")
        # Notify current user
        NotificationManager.create_notification(db, user.id, "Asset Alert", 
                                                f"You have marked '{name}' as UNSERVICEABLE.", "warning")
        db.commit()
    elif serviceability == "S":
        # Notify admins
        NotificationManager.broadcast_to_role(db, "sysadmin", "Asset Restored", 
                                              f"Asset '{name}' has been restored to SERVICEABLE status.", "success")
        # Notify current user
        NotificationManager.create_notification(db, user.id, "Asset Restored", 
                                                f"You have restored '{name}' to SERVICEABLE status.", "success")
        db.commit()
...
def delete_equipment(
    ...
    log_action(db, "DELETE", target_table="equipments", target_id=eq_id, description=f"Permanently removed asset: {eq.name}")
    db.delete(eq)
    db.commit()

    if "application/json" in request.headers.get("accept", ""):
        return {"message": "Equipment deleted successfully", "id": eq_id}
...
def get_equipment(eq_id: int, db: Session = Depends(get_db), user=Depends(require_user)):
    ...
    latest_meta = db.query(EquipmentMetadata)\
        .filter(EquipmentMetadata.equipment_id == eq.id)\
        .order_by(EquipmentMetadata.recorded_at.desc()).first()
